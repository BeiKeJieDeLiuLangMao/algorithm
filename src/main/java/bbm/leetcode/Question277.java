...
public class Question277 {
    ...
    public int findCelebrity(int n) {
        int targetMan = -1;
        for (int i = 0; i < n; i++) {
            if (findOneWhichNotKnowAnybody(n, i)) {
                // find a person who doesn't know anybody
                targetMan = i;
            }
        }
        if (targetMan == -1) {
    ...
    private boolean findOneWhichNotKnowAnybody(int n, int iAm) {
        for (int i = 0; i < n; i++) {
            if (i != iAm) {
                if (knows(iAm, i)) {
                    return false;
    ...
    public static void main(String[] args) {
        System.out.println(new Question277().findCelebrity(3));
    }
}
